...
function append_blob(directory, path, blob) {
  ...
  return new window.Response(chunk_blob)
    .arrayBuffer()
...
function write_blob(options) {
  const { path, directory, blob, recursive, on_fallback } = options;
  ...
  return Promise.all([
    ...
    .then(function on_success([config, file_info]) {
      ...
      return Promise.all([
        fetch(base_url + absolute_path + (recursive ? "?recursive=true" : ""), {
...
export async function downloadFile(options) {
  ...
  const absolute_path = file_info.uri.replace("file://", "");
  return BlobWriter.downloadFile({ ...options, absolute_path });
}
